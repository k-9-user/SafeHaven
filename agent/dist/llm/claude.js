...
const PROMPT_TEMPLATE = fs.readFileSync(path.join(__dirname, 'system-prompt.md'), 'utf8');
...
export function buildSystemPrompt(ctx = {}) {
    const { locale = 'en', userTier = 'novice', riskScore = 1, goals = [], monthlyCapitalUSD = 0, walletType = 'starter', voiceMode = false, conversationSummary, } = ctx;
    const languageName = LOCALE_NAMES[locale] ?? 'English';
    const goalsText = goals.length > 0 ? goals.join(', ') : 'not yet specified';
    const capitalText = monthlyCapitalUSD === 0
...
const FINANCIAL_DISTRESS_PATTERNS = [
...
const TOOLS = [
    {
        ...
        description: "Recommend a conservative DeFi savings strategy. Only call after collecting: the user's capital range, goal, time horizon, and loss tolerance. Always include APY as a range, never a fixed number.",
        input_schema: {
            type: 'object',
            properties: {
                strategyId: {
    ...
    {
        name: 'confirm_action',
        description: 'Request explicit user confirmation before any on-chain action (deposit, withdraw, bridge, sign). Always call this before describing a transaction. Include a plain-language summary and fee estimate.',
        input_schema: {
            type: 'object',
            properties: {
    ...
    {
        name: 'flag_distress',
        description: "Call when the user shows signs of financial distress, scam victimisation, compulsive behaviour, or emotional crisis. This triggers the app's support protocol — do not recommend new investments after calling this.",
        input_schema: {
            type: 'object',
            properties: {
...
export async function* streamChat(messages, ctx = {}) {
 ...
 */
export async function summariseHistory(turns, locale = 'en') {
    ...
    const response = await client.messages.create({
        model: MODEL,
        max_tokens: SUM_TOK,
        system: `You are summarising a financial coaching conversation for context injection. ` +
