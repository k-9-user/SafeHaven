...
import { z } from 'zod';
import { summariseHistory, type SupportedLocale } from './claude.js';
...
memoryRouter.post('/summarize', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const summary = await summariseHistory(safeMessages, locale as SupportedLocale);
...
memoryRouter.get('/summarize/health', (_req: Request, res: Response) => {
