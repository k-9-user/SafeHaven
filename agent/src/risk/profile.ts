...
function weightToRawScore(weight: number): number {
...
export function computeRiskAssessment(
  ...
  const tier: RiskTier =
    finalScore === 1 ? 'ultra_conservative' :
...
function buildExplanations(
  ...
  const templates = {
    en: {
      ...
      noEmergencyFund:
        " Before earning yield, we recommend saving 3 months of expenses as an emergency fund. We'll guide you to build it first.",
      buildingFund:
        " You're building your emergency fund — great! Once it's complete, you'll unlock more earning options.",
      score1:
        " We'll start with a simple USDC savings account — safe, liquid, and growing.",
      score2:
        " We'll recommend low-risk USDC lending earning 4–8% APY — accessible anytime.",
      score3:
        " We'll show you conservative USDC lending strategies with the best safety-adjusted yields.",
    },
    fr: {
      base: `Votre score de risque est ${score} sur 5.`,
      noEmergencyFund:
        " Avant de générer des rendements, nous recommandons d'épargner 3 mois de dépenses comme fonds d'urgence. Nous vous guiderons pour le constituer.",
      buildingFund:
        " Vous constituez votre fonds d'urgence — excellent ! Une fois terminé, vous débloquerez plus d'options.",
      score1:
        " Nous commencerons avec un simple compte d'épargne USDC — sûr, liquide et croissant.",
      score2:
        " Nous recommanderons des prêts USDC à faible risque avec 4–8% APY — accessible à tout moment.",
      score3:
        " Nous vous montrerons des stratégies conservatrices de prêt USDC avec les meilleurs rendements ajustés au risque.",
    },
    es: {
      base: `Tu puntuación de riesgo es ${score} de 5.`,
      noEmergencyFund:
        " Antes de generar rendimientos, recomendamos ahorrar 3 meses de gastos como fondo de emergencia. Te guiaremos para construirlo.",
      buildingFund:
        " Estás construyendo tu fondo de emergencia — ¡excelente! Una vez completo, desbloquearás más opciones.",
      score1:
        " Comenzaremos con una cuenta de ahorro USDC simple — segura, líquida y creciendo.",
      score2:
        " Recomendaremos préstamos USDC de bajo riesgo con 4–8% APY — accesibles en cualquier momento.",
      score3:
        " Te mostraremos estrategias conservadoras de préstamo USDC con los mejores rendimientos ajustados al riesgo.",
    },
  };

  const build = (t: typeof templates['en']): string => {
    let msg = t.base;
    if (noEmergencyFund) msg += t.noEmergencyFund;
    else if (buildingFund) msg += t.buildingFund;
    if (score === 1) msg += t.score1;
    else if (score === 2) msg += t.score2;
    else msg += t.score3;
    return msg;
  };
 ...
 */
export function parseAndAssessProfile(
  raw: unknown,
): { payload: OnboardingProfilePayload; assessment: RiskAssessment } | { error: string } {
...
export interface RiskProfileHistoryEntry {
  entryId: string;           // UUID
  userId?: string;           // May be absent for anonymous users
...
export function buildHistoryEntry(
  payload: OnboardingProfilePayload,
  assessment: RiskAssessment,
  source: RiskProfileHistoryEntry['source'] = 'onboarding',
