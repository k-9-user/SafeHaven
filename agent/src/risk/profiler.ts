...
export interface RiskAssessment {
  score: number;           // 1–5
  tier: RiskTier;
  maxStrategyRiskScore: number; // Strategies with riskScore > this are hidden
  canUseDeFi: boolean;
  recommendedStrategyIds: string[];
  explanation: Record<string, string>; // Locale → explanation
}
 ...
 */
export function computeRiskScore(profile: RiskProfile): RiskAssessment {
  let score = 1;

  // Capital: more capital available → slightly higher score allowed
  switch (profile.availableCapital) {
    case 'under_50':   score += 0; break;
    case '50_200':     score += 0.5; break;
    case '200_1000':   score += 1; break;
    case 'over_1000':  score += 1.5; break;
  }

  // Time horizon: longer → slightly more tolerance for illiquidity
  switch (profile.timeHorizon) {
    case 'under_3_months': score += 0; break;
    case '3_12_months':    score += 0.25; break;
    case '1_3_years':      score += 0.5; break;
    case 'over_3_years':   score += 1; break;
  }

  // Loss tolerance: primary driver of risk score
  switch (profile.lossTolerance) {
    case 'none':      score += 0; break;
    case 'very_low':  score += 0.25; break;
  ...
  const tier: RiskTier =
    finalScore <= 2 ? 'conservative' : 'moderate';
...
export function parseRiskProfile(
  raw: unknown,
): { profile: RiskProfile; assessment: RiskAssessment } | { error: string } {
 ...
 */
export function isProfileComplete(partial: Partial<RiskProfile>): boolean {
  return (
    partial.availableCapital !== undefined &&
