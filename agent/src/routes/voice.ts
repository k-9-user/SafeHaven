...
voiceRouter.post('/synthesize/stream', async (req: Request, res: Response): Promise<void> => {
  const parsed = SynthesizeSchema.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: 'Invalid request', details: parsed.error.flatten() });
    return;
...
voiceRouter.post('/transcribe', upload.single('audio'), async (req: Request, res: Response): Promise<void> => {
  const apiKey = process.env['ELEVENLABS_API_KEY'];
  if (!apiKey) {
