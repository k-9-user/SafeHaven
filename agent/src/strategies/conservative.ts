...
export interface ConservativeStrategy {
  ...
  name: string;
  riskScore: number;           // 1–3 (conservative only)
  minDepositUSDC: number;
  apyRangeIndicative: { min: number; max: number };
  programId: string;
  vaultAddress: string;
  auditLinks: string[];
  description: {
...
export const CONSERVATIVE_STRATEGIES: ConservativeStrategy[] = [
  {
    id: 'kamino-usdc-main',
    protocol: 'kamino',
 ...
 */
export async function fetchStrategyApy(strategyId: string): Promise<number> {
...
async function fetchKaminoApy(vaultAddress: string): Promise<number> {
  // Kamino API endpoint (public)
  const response = await axios.get(
    `https://api.kamino.finance/v2/strategies/${vaultAddress}/metrics`,
    { timeout: 8_000 },
  );
  return parseFloat(response.data?.lendingApy ?? '0') * 100;
}

async function fetchMarginFiApy(_poolAddress: string): Promise<number> {
  // MarginFi API endpoint
  const response = await axios.get(
    'https://marginfi.com/api/banks',
    { timeout: 8_000 },
  );
  const usdcBank = response.data?.find(
    (b: { tokenSymbol: string; lendingRate: number }) => b.tokenSymbol === 'USDC',
  );
 ...
 */
export async function getAllYields(): Promise<
  Record<string, { apy: number; timestamp: string }>
> {
  const results: Record<string, { apy: number; timestamp: string }> = {};

  await Promise.allSettled(
    CONSERVATIVE_STRATEGIES.map(async (strategy) => {
