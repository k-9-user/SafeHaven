...
export async function reconnect(config, parameters = {}) {
    ...
    const connectionPromises = sorted.map(async (connector) => {
        try {
            ...
            const provider = await retryUntil(() => connector.getProvider().catch(() => undefined), { timeout: 5000, interval: 100 });
            if (!provider) {
                return null;
            }
            ...
            // Attempt connection
            const data = await withTimeout(() => connector.connect({ isReconnecting: true }), { timeout: 5000 });
