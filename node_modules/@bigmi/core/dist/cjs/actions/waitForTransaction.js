...
const withRetry_js_1 = require("../utils/withRetry.js");
...
async function waitForTransaction(client, { confirmations = 1, txId, txHex, senderAddress, onReplaced, pollingInterval = client.pollingInterval, retryCount = 10, retryDelay = 3000, timeout, }) {
    const observerId = (0, stringify_js_1.stringify)(['waitForTransaction', client.uid, txId]);
    let count = 0;
    let transaction;
    ...
    return new Promise((resolve, reject) => {
        if (timeout) {
            setTimeout(() => reject(new transaction_js_1.WaitForTransactionReceiptTimeoutError({ hash: txId })), timeout);
        }
        const _unobserve = (0, observe_js_1.observe)(observerId, { onReplaced, resolve, reject }, (emit) => {
            const _unwatch = (0, getAction_js_1.getAction)(client, watchBlockNumber_js_1.watchBlockNumber, 'watchBlockNumber')({
                ...
                async onBlockNumber(blockNumber_) {
                    ...
                    if (count > retryCount) {
                        done(() => emit.reject(new transaction_js_1.WaitForTransactionReceiptTimeoutError({
                            hash: txId,
                        })));
                    }
                    try {
                        if (transaction?.blockhash) {
                            const blockStats = await (0, getAction_js_1.getAction)(client, getBlockStats_js_1.getBlockStats, 'getBlockStats')({
                        ...
                        retrying = true;
                        transaction = await (0, withRetry_js_1.withRetry)(() => (0, getAction_js_1.getAction)(client, getUTXOTransaction_js_1.getUTXOTransaction, 'getUTXOTransaction')({ txId: transaction?.txid || txId }), {
                            delay: retryDelay,
                            retryCount,
                        });
                        if (transaction.blockhash) {
                        ...
                        if (!transaction?.confirmations) {
                            throw new transaction_js_1.TransactionReceiptNotFoundError({
                                hash: txId,
                            });
                        }
                        ...
                        if (err instanceof transaction_js_1.TransactionNotFoundError ||
                            ...
                            try {
                                ...
                                transaction = await (0, getAction_js_1.getAction)(client, getUTXOTransaction_js_1.getUTXOTransaction, 'getUTXOTransaction')({
