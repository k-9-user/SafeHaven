...
async function reconnect(config, parameters = {}) {
	...
	const connectionPromises = sorted.map(async (connector) => {
		try {
			const provider = await (0, _bigmi_core.retryUntil)(() => connector.getProvider().catch(() => void 0), {
				timeout: 5e3,
				interval: 100
			});
			...
			if (!await (0, _bigmi_core.withTimeout)(() => connector.isAuthorized(), { timeout: 5e3 })) return null;
			const data = await (0, _bigmi_core.withTimeout)(() => connector.connect({ isReconnecting: true }), { timeout: 5e3 });
