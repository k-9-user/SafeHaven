...
function leather(parameters = {}) {
	...
	return require_factories_createConnector.createConnector((config) => ({
		...
		async connect() {
			if (!await this.getInternalProvider()) throw new _bigmi_core.ProviderNotFoundError();
			try {
		...
		async getAccounts() {
			...
			if (!accounts.result) throw new _bigmi_core.UserRejectedRequestError(accounts.error?.message ?? "Unknown error");
