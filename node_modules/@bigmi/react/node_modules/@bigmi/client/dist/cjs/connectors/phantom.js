...
//#region src/connectors/phantom.ts
	...
	return require_factories_createConnector.createConnector((config) => ({
		...
		async getInternalProvider() {
			...
			if ("phantom" in window) {
				const internalProvider = window.phantom?.bitcoin;
				if (internalProvider?.isPhantom) return internalProvider;
			}
		},
		async getProvider() {
		...
		async request({ method, params }) {
			switch (method) {
				case "signPsbt": {
					...
					return Array.from(signedPsbt, (byte) => byte.toString(16).padStart(2, "0")).join("");
				}
		...
		async getAccounts() {
			const provider = await this.getInternalProvider();
			if (!provider) throw new _bigmi_core.ProviderNotFoundError();
			return (await provider.requestAccounts()).filter((account) => account.purpose === "payment");
		},
		async getChainId() {
