...
const require_factories_createConnector = require("../factories/createConnector.js");
const require_utils_createBidirectionalMap = require("../utils/createBidirectionalMap.js");
let _bigmi_core = require("@bigmi/core");
//#region src/connectors/unisat.ts
let UnisatBitcoinChainEnum = /* @__PURE__ */ function(UnisatBitcoinChainEnum) {
	UnisatBitcoinChainEnum["BITCOIN_MAINNET"] = "BITCOIN_MAINNET";
	UnisatBitcoinChainEnum["BITCOIN_TESTNET"] = "BITCOIN_TESTNET";
	UnisatBitcoinChainEnum["BITCOIN_TESTNET4"] = "BITCOIN_TESTNET4";
	UnisatBitcoinChainEnum["BITCOIN_SIGNET"] = "BITCOIN_SIGNET";
	UnisatBitcoinChainEnum["FRACTAL_BITCOIN_MAINNET"] = "FRACTAL_BITCOIN_MAINNET";
	UnisatBitcoinChainEnum["FRACTAL_BITCOIN_TESTNET"] = "FRACTAL_BITCOIN_TESTNET";
	return UnisatBitcoinChainEnum;
}({});
function unisat(parameters = {}) {
	const UnisatBitcoinNetworkChainIdMap = {
		livenet: _bigmi_core.ChainId.BITCOIN_MAINNET,
		testnet: _bigmi_core.ChainId.BITCOIN_TESTNET
	};
	const { forward: UnisatBitcoinChainIdMap, reverse: ChainIdToUnisatMap } = require_utils_createBidirectionalMap.createBidirectionalMap([
		[UnisatBitcoinChainEnum.BITCOIN_MAINNET, _bigmi_core.ChainId.BITCOIN_MAINNET],
		[UnisatBitcoinChainEnum.BITCOIN_TESTNET, _bigmi_core.ChainId.BITCOIN_TESTNET],
		[UnisatBitcoinChainEnum.BITCOIN_TESTNET4, _bigmi_core.ChainId.BITCOIN_TESTNET4],
		[UnisatBitcoinChainEnum.BITCOIN_SIGNET, _bigmi_core.ChainId.BITCOIN_SIGNET],
		[UnisatBitcoinChainEnum.FRACTAL_BITCOIN_MAINNET, _bigmi_core.ChainId.FRACTAL_BITCOIN_MAINNET],
		[UnisatBitcoinChainEnum.FRACTAL_BITCOIN_TESTNET, _bigmi_core.ChainId.FRACTAL_BITCOIN_TESTNET]
	]);
	const { shimDisconnect = true } = parameters;
	let accountsChanged;
	let chainChanged;
	return require_factories_createConnector.createConnector((config) => ({
		id: "unisat",
		name: "Unisat",
		type: unisat.type,
		...
		async getInternalProvider() {
			...
			if ("unisat" in window) {
				...
				if (anyWindow.unisat?.isBinance || anyWindow.unisat?.isBitKeep) return;
		...
		async request({ method, params }) {
			switch (method) {
				...
				default: throw new _bigmi_core.MethodNotSupportedRpcError();
			}
		},
		async connect() {
			const provider = await this.getInternalProvider();
			if (!provider) throw new _bigmi_core.ProviderNotFoundError();
			try {
				...
				if (!chainChanged) {
					chainChanged = (network) => this.onChainChanged(UnisatBitcoinNetworkChainIdMap[network]);
				...
				throw new _bigmi_core.UserRejectedRequestError(error.message);
			}
		},
		async disconnect() {
			const provider = await this.getInternalProvider();
		...
		async getAccounts() {
			...
			const { type, purpose } = (0, _bigmi_core.getAddressInfo)(address);
			return [{
				address,
				addressType: type,
				publicKey,
		...
		async getChainId() {
			const provider = await this.getInternalProvider();
			if (!provider) throw new _bigmi_core.ProviderNotFoundError();
			return UnisatBitcoinChainIdMap[(await provider.getChain()).enum];
		},
		async isAuthorized() {
			try {
		...
		async switchChain({ chainId }) {
			try {
				...
				if (!unisatChainId) throw new require_errors_connectors.ChainNotSupportedError(chainId, unisat.name);
...
unisat.type = "UTXO";
//#endregion
exports.UnisatBitcoinChainEnum = UnisatBitcoinChainEnum;
