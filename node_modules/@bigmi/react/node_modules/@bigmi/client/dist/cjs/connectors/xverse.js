...
const require_utils_debounce = require("../utils/debounce.js");
let _bigmi_core = require("@bigmi/core");
//#region src/connectors/xverse.ts
function xverse(parameters = {}) {
	const XverseBitcoinChainIdMap = {
		Mainnet: _bigmi_core.ChainId.BITCOIN_MAINNET,
		Testnet: _bigmi_core.ChainId.BITCOIN_TESTNET,
		Testnet4: _bigmi_core.ChainId.BITCOIN_TESTNET4,
		Signet: _bigmi_core.ChainId.BITCOIN_SIGNET
	};
	...
	return require_factories_createConnector.createConnector((config) => ({
		...
		async request({ method, params }) {
			switch (method) {
				case "signPsbt": {
					...
					const psbtBase64 = (0, _bigmi_core.hexToBase64)(psbt);
					...
					return (0, _bigmi_core.base64ToHex)(signedPsbt?.result?.psbt);
				}
				default: throw new _bigmi_core.MethodNotSupportedRpcError(method);
			}
		},
		async connect({ isReconnecting } = {}) {
			...
			if (!isReconnecting) {
				...
				if (connected.error) throw new _bigmi_core.UserRejectedRequestError(connected.error.message);
			}
			...
			if (!chainChange) {
				chainChange = require_utils_debounce.debounce((event) => this.onChainChanged(XverseBitcoinChainIdMap[event.bitcoin.name]), 300);
		...
		async getAccounts() {
			...
			if (!accounts.result) throw new _bigmi_core.UserRejectedRequestError(accounts.error?.message);
		...
		async getChainId() {
			...
			if (!network.result) throw new _bigmi_core.UserRejectedRequestError(network.error?.message ?? "Unknown error");
