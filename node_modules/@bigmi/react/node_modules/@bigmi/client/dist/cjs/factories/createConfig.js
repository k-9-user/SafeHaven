...
const require_errors_config = require("../errors/config.js");
const require_factories_createEmitter = require("./createEmitter.js");
const require_factories_createStorage = require("./createStorage.js");
...
function createConfig(parameters) {
	const { multiInjectedProviderDiscovery = true, storage = require_factories_createStorage.createStorage({
		key: "bigmi",
		storage: require_factories_createStorage.getDefaultStorage()
	}), syncConnectedChain = true, ssr = false, ...rest } = parameters;
	const chains = (0, zustand_vanilla.createStore)(() => rest.chains);
	const connectors = (0, zustand_vanilla.createStore)(() => {
		const collection = [];
	...
	function setup(connectorFn) {
		const emitter = require_factories_createEmitter.createEmitter((0, _bigmi_core.uid)());
		const connector = {
			...connectorFn({
				emitter,
	...
	function getClient(config = {}) {
		...
		if (config.chainId && !chain) throw new require_errors_config.ChainNotConfiguredError();
		{
			const client = clients.get(store.getState().chainId);
			if (client && !chain) return client;
			if (!chain) throw new require_errors_config.ChainNotConfiguredError();
		}
		{
		...
		if (rest.client) client = rest.client({ chain });
			...
			client = (0, _bigmi_core.createClient)({
				...properties,
				chain,
	...
	if (_bigmi_core.version.startsWith(prefix)) currentVersion = Number.parseInt(_bigmi_core.version.replace(prefix, ""), 10);
	else currentVersion = Number.parseInt(_bigmi_core.version.split(".")[0] ?? "0", 10);
	const store = (0, zustand_vanilla.createStore)((0, zustand_middleware.subscribeWithSelector)(storage ? (0, zustand_middleware.persist)(getInitialState, {
