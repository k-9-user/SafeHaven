...
import { ChainId, MethodNotSupportedRpcError, ProviderNotFoundError, UserRejectedRequestError, getAddressInfo } from "@bigmi/core";
//#region src/connectors/binance.ts
function binance(parameters = {}) {
	const { forward: BinanceBitcoinNetworkChainIdMap, reverse: ReverseChainIdMap } = createBidirectionalMap([
	...
	return createConnector((config) => ({
		...
		async getInternalProvider() {
			...
			if ("binancew3w" in window) return window.binancew3w.bitcoin;
			if ("unisat" in window) {
				const anyWindow = window;
				if (anyWindow.unisat.isBinance) return anyWindow.unisat;
			}
		},
		async getProvider() {
		...
		async connect() {
			...
			try {
				...
				if (!chainChanged) {
					chainChanged = (network) => this.onChainChanged(BinanceBitcoinNetworkChainIdMap[network]);
		...
		async getChainId() {
			const provider = await this.getInternalProvider();
			if (!provider) throw new ProviderNotFoundError();
			return BinanceBitcoinNetworkChainIdMap[await provider.getNetwork()];
		},
		async isAuthorized() {
			try {
		...
		async switchChain({ chainId }) {
			try {
				...
				if (!network) throw new ChainNotSupportedError(chainId, binance.name);
