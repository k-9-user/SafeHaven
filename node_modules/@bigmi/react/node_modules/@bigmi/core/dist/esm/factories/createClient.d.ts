...
import { BitcoinRpcMethods } from "../types/rpc.js";
...
type ClientConfig<transport extends Transport = Transport, chain extends Chain | undefined = Chain | undefined, accountOrAddress extends Account | Address | undefined = Account | Address | undefined, rpcSchema extends RpcSchema | undefined = undefined> = {
  /** The Account to use for the Client. This will be used for Actions that require an account as an argument. */account?: accountOrAddress | Account | Address | undefined; /** Flags for batch settings. */
  batch?: {
    /** Toggle to enable `eth_call` multicall aggregation. */multicall?: boolean | Prettify<MulticallBatchOptions> | undefined;
  } | undefined;
  /**
  * Time (in ms) that cached data will remain in memory.
  * @default 4_000
  */
  cacheTime?: number | undefined; /** Chain for the client. */
  chain?: Chain | undefined | chain; /** A key for the client. */
  key?: string | undefined; /** A name for the client. */
  ...
  */
  rpcSchema?: rpcSchema | undefined; /** The RPC transport */
  transport: transport; /** The type of client. */
...
type Client_Base<transport extends Transport = Transport, chain extends Chain | undefined = Chain | undefined, account extends Account | undefined = Account | undefined, rpcSchema extends RpcSchema | undefined = undefined> = {
  /** The Account of the Client. */account: account; /** Time (in ms) that cached data will remain in memory. */
  cacheTime: number; /** Chain for the client. */
  chain: chain; /** A key for the client. */
  key: string; /** A name for the client. */
  name: string; /** Frequency (in ms) for polling enabled actions & events. Defaults to 4_000 milliseconds. */
  pollingInterval: number; /** Request function wrapped with friendly error handling */
  request: BtcRpcRequestFn<rpcSchema extends undefined ? BitcoinRpcMethods : rpcSchema>; /** The RPC transport */
  transport: ReturnType<transport>["config"] & ReturnType<transport>["value"]; /** The type of client. */
  type: string; /** A unique ID for the client. */
  uid: string;
};
...
type MulticallBatchOptions = {
  /** The maximum size (in bytes) for each calldata chunk. @default 1_024 */batchSize?: number | undefined; /** The maximum number of milliseconds to wait before sending a batch. @default 0 */
