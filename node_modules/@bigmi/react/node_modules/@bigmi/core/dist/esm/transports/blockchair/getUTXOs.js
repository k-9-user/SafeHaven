...
const getUTXOs = async (client, { baseUrl, apiKey }, { address, minValue }) => {
	async function* fetchUTXOs() {
		...
		while (hasMore) {
			...
			if (response.context?.code !== 200 && response.context?.code !== 404) throw new RpcRequestError({
				url: apiUrl,
				body: {
					method: "fetchUTXOs",
					params: {
	...
	if (minValue) {
		const { error, result: balance } = await getBalance(client, {
			baseUrl,
			apiKey
		}, { address });
		if (error) throw new BaseError("Error fetching balance", { cause: error });
		if (balance === void 0) throw new BaseError("Balance is undefined", { cause: /* @__PURE__ */ new Error("Unable to determine balance") });
		if (minValue > Number(balance)) throw new InsufficientUTXOBalanceError({
			minValue,
			address,
