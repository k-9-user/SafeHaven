...
export function useCallsHistory(parameters = {}) {
    const { account: accountParam, connector: connectorParam, query = {}, ...rest } = parameters;
    const config = useConfig(rest);
    const { address, connector, status } = useAccount();
    ...
    return useQuery({
        ...
        enabled,
        gcTime: 0,
        queryFn: account && activeConnector
            ? async (context) => {
                const [, { connectorUid: _, ...options }] = context.queryKey;
